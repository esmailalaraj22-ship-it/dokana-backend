...
const tableConfig = getTableConfig(suppliers);
...
function mappedCheckSql(value: SQL): string {
  return dialect
    .sqlToQuery(value)
    .sql.replaceAll('"ledger"."suppliers".', '')
    .replaceAll('"', '')
    .replace(/\s+/g, ' ')
    .trim();
}
...
function mappedIndexColumnName(column: object): string {
  if (!('name' in column) || typeof column.name !== 'string') {
    throw new TypeError('Supplier indexes must use named physical columns.');
  }
  return column.name;
}

describe('Supplier database contract', () => {
  ...
  beforeAll(() => {
    ...
    adminPool = createTestPool(environment.adminUrl, 'dokana-task61-contract', 1);
    const sqliteSource = resolve(
      process.cwd(),
      'database/reference/backend_database_reference/sqlite_shop_ledger_schema_v1_1_empty.db',
    );
    sqliteTempDirectory = mkdtempSync(join(tmpdir(), 'dokana-task61-'));
    const sqliteCopy = join(sqliteTempDirectory, 'supplier-contract.db');
  ...
  it('maps every PostgreSQL Supplier column, default, nullability, and identity rule exactly', async () => {
    const catalog = await pool().query<CatalogColumnRow>(`
      ...
      where attribute.attrelid = 'ledger.suppliers'::regclass
    ...
    expect(suppliers.id.hasDefault).toBe(false);
    expect(suppliers.phone.notNull).toBe(false);
    expect(suppliers.normalizedPhone.notNull).toBe(false);
    expect(suppliers.version.dataType).toBe('bigint');
  });

  it('matches PostgreSQL constraints, indexes, and tenant-safe foreign keys exactly', async () => {
    const uniqueConstraints = await pool().query<NamedColumnsRow>(`
      ...
      where constraint_state.conrelid = 'ledger.suppliers'::regclass
    ...
    const indexes = await pool().query<NamedColumnsRow & { unique: boolean }>(`
      ...
      where index_state.indrelid = 'ledger.suppliers'::regclass
    ...
    const checks = await pool().query<{ name: string }>(`
      ...
      from pg_constraint
      where conrelid = 'ledger.suppliers'::regclass
        and contype = 'c'
      order by conname
    `);
    ...
    expect(
      ...
      suppliers_name_check: 'length(trim(name)) > 0',
      suppliers_normalized_name_check: 'length(trim(normalized_name)) > 0',
      suppliers_status_check: "status in ('active', 'archived')",
      suppliers_check: "(status = 'archived' and archived_at is not null) or status = 'active'",
      suppliers_version_check: 'version >= 1',
    });
    expect(uniqueConstraints.rows).toContainEqual({
      name: 'suppliers_store_id_normalized_phone_key',
      columns: ['store_id', 'normalized_phone'],
    });
  });
  ...
  it('verifies server-only RLS, trigger, ownership, and incoming-reference behavior', async () => {
    const relation = await pool().query<{
      ...
      where relation.oid = 'ledger.suppliers'::regclass
    `);
    const policies = await pool().query<{
      ...
      from pg_policies
      where schemaname = 'ledger' and tablename = 'suppliers'
      order by policyname
    `);
    const triggers = await pool().query<{
      ...
      where trigger_state.tgrelid = 'ledger.suppliers'::regclass
    ...
    const incomingReferences = await pool().query<{
      ...
      select
        ...
        constraint_state.conname as name
      from pg_constraint as constraint_state
      where constraint_state.confrelid = 'ledger.suppliers'::regclass
    ...
    const runtimePrivileges = await pool().query<{
      ...
      select
        has_table_privilege('dokana_runtime_login', 'ledger.suppliers', 'select')
          as "selectAllowed",
        has_table_privilege('dokana_runtime_login', 'ledger.suppliers', 'insert')
          as "insertAllowed",
        has_table_privilege('dokana_runtime_login', 'ledger.suppliers', 'update')
          as "updateAllowed",
        has_table_privilege('dokana_runtime_login', 'ledger.suppliers', 'delete')
          as "deleteAllowed",
        (select rolsuper from pg_roles where rolname = 'dokana_runtime_login')
          as "isSuperuser",
        (select rolbypassrls from pg_roles where rolname = 'dokana_runtime_login')
          as "bypassRls",
        pg_get_userbyid((select relowner from pg_class where oid = 'ledger.suppliers'::regclass))
          = 'dokana_runtime_login' as "tableOwner",
        pg_has_role('dokana_runtime_login', 'shop_app_migrator', 'member') as "migratorMember",
        pg_has_role('dokana_runtime_login', 'shop_app_migrator', 'set') as "canSetMigrator"
    `);
    ...
    expect(policies.rows).toEqual([
      {
        name: 'tenant_isolation_suppliers',
        command: 'ALL',
    ...
    expect(incomingReferences.rows).toEqual([
      {
        sourceTable: 'ledger.goods_receipts',
        name: 'goods_receipts_store_id_supplier_id_fkey',
      },
      {
        sourceTable: 'ledger.purchase_invoices',
        name: 'purchase_invoices_store_id_supplier_id_fkey',
      },
      {
        sourceTable: 'ledger.supplier_ledger_entries',
        name: 'supplier_ledger_entries_store_id_supplier_id_fkey',
      },
      {
        sourceTable: 'ledger.supplier_payments',
        name: 'supplier_payments_store_id_supplier_id_fkey',
      },
      {
        sourceTable: 'ledger.supplier_returns',
  ...
  it('matches the approved SQLite shared Supplier representation read-only', async () => {
    ...
    const searchIndexColumns = sqlite()
      .prepare("pragma index_info('idx_suppliers_search')")
      .all() as unknown as { name: string }[];
    const noDeleteTrigger = sqlite()
      .prepare(
        "select name from sqlite_master where type = 'trigger' and tbl_name = 'suppliers' order by name",
      )
      .all();
    ...
    expect(noDeleteTrigger).toEqual([{ name: 'trg_suppliers_no_delete' }]);
  });
});
